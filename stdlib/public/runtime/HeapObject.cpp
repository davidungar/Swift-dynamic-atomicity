...
SWIFT_RT_ENTRY_IMPL(swift_allocObject)(HeapMetadata const *metadata,
  ...
  void const * const visitorOfRefsInHeapObj_dmu_ = reinterpret_cast<void*>(asFullMetadata(object->metadata)->visitorOfRefsInHeapObj_dmu_);
  if (    VisitorOfRefsInHeapObj_dmu_Values::unimplemented
      ==  visitorOfRefsInHeapObj_dmu_ ) {
...
public:
  ...
  : Data{HeapMetadataHeader{{
    destroyGenericBox,
    (void (*)(HeapObject*))(VisitorOfRefsInHeapObj_dmu_Values::unimplemented)
  }, {/*vwtable*/ nullptr}},
...
bool swift::swift_unownedIsDestSafeForConcurrentAccess_dmu_(HeapObject *dst)
SWIFT_CC(RegisterPreservingCC_IMPL) {
  return SWIFT_RT_ENTRY_CALL(swift_isDestSafeForConcurrentAccess_dmu_(dst));
}
...
bool swift::swift_weakIsDestSafeForConcurrentAccess_dmu_(WeakReference *dst) {
  return
    dst == nullptr                    ?  false
  : !isNativeSwiftWeakReference(dst)  ?  true
  : _swift_isDestSafeForConcurrentAccess_dmu__((HeapObject*)(dst->Value & ~WR_NATIVE));
}
