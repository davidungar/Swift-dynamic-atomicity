...
bool ide::printDeclUSR(const ValueDecl *D, raw_ostream &OS) {
  ...
  if (isa<ParamDecl>(VD)
      && (
          ...
          || isa<VisitRefsInInstance_dmu_Decl>(VD->getDeclContext())))
    return true;
  ...
  } else if (auto Maker = dyn_cast<VisitRefsInInstance_dmu_Decl>(VD)) {
    Mangler.mangleVisitRefsInInstance_dmu_Entity(Maker);
