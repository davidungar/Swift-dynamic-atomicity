...
CONSTANT_OWNERSHIP_INST(Trivial, false, VisitRefAtAddr_dmu_)
...
OwnershipUseCheckerResult
OwnershipCompatibilityUseChecker::visitStoreBarrier_dmu_Inst(StoreBarrier_dmu_Inst *I) {
  if (getValue() == I->getSrc())
    return {compatibleWithOwnership(ValueOwnershipKind::Owned), true};
  return {true, false};
}
...
CONSTANT_OWNERSHIP_BUILTIN(Trivial, false, VisitRefsInArray_dmu_)
...
BUILTINS_THAT_SHOULD_HAVE_BEEN_LOWERED_TO_SILINSTS(VisitRefs_dmu_)
